...
- [x] 支持IPv6代理路由规则配置
- ~~[x] 支持Clash内核切换： Clash Premium/Clash.Meta **内核切换按钮**功能。~~
- [x] 只使用 Clash.Meta 内核+TPROXY模式，dns污染问题由 自定义dnsmasq默认DNS和分流规则解决。
- [ ] 支持**启动配置**订阅 : 方便随时切换订阅源，但我还是建议自己实现本地配置，这样更适合自己需求。
- [ ] 支持 **代理节点组** 订阅: 目前修改启动配置实现，接下来通过输入框增加代理节点组订阅。
